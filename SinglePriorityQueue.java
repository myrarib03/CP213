...
public class SinglePriorityQueue<T extends Comparable<T>> extends SingleLink<T> {
	...
	while (currentLeft != null || currentRight != null) {
	    ...
	    if (currentRight == null
		    || (currentLeft != null && currentLeft.getObject().compareTo(currentRight.getObject()) <= 0)) {
	    ...
	    if (this.rear == null) {
		this.front = this.rear = nextNode;
	    } else {
		this.rear.setNext(nextNode);
		this.rear = nextNode;
	    }
	    this.length++;
	}
	...
	T highestPriorityObject = this.front.getObject();
	...
	this.length = 0;
	while (current != null) {
	    SingleNode<T> nextNode = current.getNext();
